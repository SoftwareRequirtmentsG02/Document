			...
			if($action == 1)                                     //判断操作类型，“1”为恢复操作，“2”为删除操作
			{
				...
				if(preg_match($preg1,$name,$str1))              //如果恢复的是.zip格式的备份
				{
				    ...
				    if($this->exactZipDb($filename))
				    {
					    ...
					    chmod($sqlname,0777);                     //added by wmc 2010/06/24
 ...
 */
	private function exactZipDb($file)
	{
        ...
        if ($res === TRUE) 
         {
         	$tempath = "./system/store/temp/";
         	if(!file_exists($tempath)){            //added by wmc 2010/06/24
         		mkdir($tempath,0777);
         		chmod($tempath,0777);
         	}	
           $zip->extractTo($tempath);    //将zip文档解压到“system/store/temp”文件夹下
           $zip->close();
           return 1;
